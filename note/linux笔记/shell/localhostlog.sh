...
fdisk -l | grep "Disk /dev/sd" | awk -F '[ :,]+' '{printf "%.0f\n",$5/1072741824}' | awk -v total=0 '{total+=$1}END{printf "磁盘：%.0fG\t",total}' >> $fname
df -k | grep -v "tmpfs" | egrep -A 1 "mapper|sd" | awk 'NF>1{print $(NF-3)}' | awk -v used=0 '{used+=$1}END{printf "使用：%.2f%\n",used/1048576}' >> $fname
#free
free -g | sed -n '2p' | awk '{print "总共：",$2,"使用：",$3,"剩余：",$4,"G"}' >> $fname
echo '-----------------------------------------------------------------------------------------------------正在运行的python脚本' >> $fname
ps -ef | grep 'python' | grep 'start$' | awk '{print $8,$9}' >> $fname
echo "--------------------------------------------------------------------------------------------------------------------------" >> $fname
#log
for i in ${logname[@]}
do
